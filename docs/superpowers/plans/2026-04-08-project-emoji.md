...
```ts
const { project, field } = (
  e as CustomEvent<{ project: Project; field: "name" | "emoji" | "dueDate" }>
).detail;
```
        ...
        <div className="flex items-center h-11 px-3 gap-2 border border-transparent focus-within:border-accent transition-colors duration-150">
          ...
          >
            {emoji ? (
              ...
              <>
                <span style={{ fontSize: "10px", opacity: 0.55 }}>@c</span>
                &nbsp;icon
              </>
            )}
          </button>
          ...
          >
            {dueDate ? (
              <>
                <span style={{ fontSize: "10px", opacity: 0.55 }}>@d</span>
                &nbsp;{formatDate(dueDate)}
              </>
            ) : (
              <>
...
```tsx
{
  showProject && (
    ...
    >
      {project.emoji && (
        <span className="text-[12px] shrink-0">{project.emoji}</span>
      )}
