...
describe('Dropdown Keyboard Navigation', () => {
  it('navigates the date dropdown with arrow keys', async () => {
    ...
    // Press ArrowRight (today April 10 + 1 = April 11)
    fireEvent.keyDown(window, { key: 'ArrowRight' });
    expect(screen.getByRole('button', { name: 'dueDate' })).toHaveTextContent('11/04/2026');
    
    // 2. Open it again (query is reset to empty)
    fireEvent.click(screen.getByRole('button', { name: 'dueDate' }));
    
    // Press ArrowDown (today April 10 + 7 = April 17)
    fireEvent.keyDown(window, { key: 'ArrowDown' });
    expect(screen.getByRole('button', { name: 'dueDate' })).toHaveTextContent('17/04/2026');
  });
});
